...
(* pair_to_int :
  ...
  * * * * *
  *)
let rec pair_to_int dim i j = 
  assert(dim > 0);
  assert(i >= 0);
  (* below -1: we start numbering 0 at (0,1) *)
  if i=0 then (assert(1<=j && j<dim); j-1)
  else dim-1+(pair_to_int (dim-1) (i-1) (j-1))
